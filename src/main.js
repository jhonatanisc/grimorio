...
function renderRecipes() {
  ...
  elements.recipeList.innerHTML =
    filtered
      .map(
        (recipe) =>
          `<article class="recipe-card"><div class="recipe-top"><span class="recipe-title">${escapeHtml(recipe.name)}</span><button class="btn compact icon-action favorite" data-favorite="${recipe.id}" aria-label="${state.favorites.includes(recipe.id) ? "Quitar de favoritas" : "Marcar favorita"}" title="Favorita">${state.favorites.includes(recipe.id) ? "★" : "☆"}</button></div><div class="recipe-ingredients">${recipe.ingredients.slice(0, 5).map(ingredientName).map(escapeHtml).join(" · ")}</div><div class="tag-row">${recipe.proteinTypes.map((x) => `<span class="tag">${escapeHtml(x)}</span>`).join("")}</div><div class="card-actions"><button class="btn compact icon-action" data-preview="${recipe.id}" aria-label="Ver detalle" title="Ver detalle">👁</button><button class="btn compact primary icon-action" data-assign="${recipe.id}" aria-label="Asignar receta" title="Asignar">＋</button><button class="btn compact icon-action" data-edit="${recipe.id}" aria-label="Editar receta" title="Editar">✎</button>${recipe.id.startsWith("custom-") ? `<button class="btn compact danger icon-action" data-delete="${recipe.id}" aria-label="Eliminar receta" title="Eliminar">⌫</button>` : ""}</div></article>`,
      )
