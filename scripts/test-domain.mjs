...
const equivalences = JSON.parse(
  await readFile(new URL("../data/equivalences.json", import.meta.url)),
);
...
const dinnerCompatible = recipes.find(
  (recipe) => recipe.mealTypes.includes("cena") && recipe.id !== breakfast.id,
);
...
assert.deepEqual(
  ...
  menu,
  "Importación/exportación no conserva el menú",
);

assert(
  recipes.every((recipe) => !recipe.mealTypes.includes("comida") || recipe.mealTypes.length === 1),
  "Una comida se asignó a otro tiempo",
);
assert(
  recipes.every(
    (recipe) => !recipe.mealTypes.includes("colacion") || recipe.mealTypes.length === 1,
  ),
  "Una colación se asignó a otro tiempo",
);
assert(
  recipes
    .filter((recipe) => recipe.mealTypes.includes("desayuno"))
    .every((recipe) => recipe.mealTypes.includes("cena")),
  "Desayuno no intercambiable con cena",
);
...
assert(
  ![...ids].some((id) => ["sal", "pimienta"].includes(id)),
  "Sal o pimienta aparecieron en la compra",
);
assert(
  ![...ids].some((id) => /\d+\s*(g|pieza|taza|lata)/i.test(equivalences[id].name)),
  "La compra muestra cantidades",
);

console.log(
  "OK: asignar, cambiar, eliminar, persistir, importar/exportar y compra única sin cantidades.",
);
